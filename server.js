...
// Only create HTTP server and Socket.IO if NOT on Vercel
let server = null;
let io = null;
...
if (!IS_VERCEL) {
  ...
  // On Vercel, create a dummy io for compatibility
  global.io = {
    to: () => ({ emit: () => {} }),
    emit: () => {},
    on: () => {},
...
// Store active users and locations (these won't persist across serverless function calls on Vercel)
...
if (!fs.existsSync(uploadsDir)) {
  try {
    ...
    console.log('⚠️ Cannot create uploads directory on Vercel');
  }
}
...
app.get('/', (req, res) => {
  res.json({
    success: true,
    message: 'CristRide Ride Sharing API is working on Vercel!',
    version: '1.0.0',
    status: 'operational',
    environment: IS_VERCEL ? 'vercel-serverless' : 'traditional-server',
    websockets: IS_VERCEL ? 'disabled (not supported on Vercel)' : 'enabled',
    endpoints: {
      api: '/api',
      health: '/api/health',
      test: '/api/test',
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      websockets: !IS_VERCEL && io ? 'enabled' : 'disabled'
    });
  } catch (error) {
    res.json({
      success: false,
...
app.get('/api/server-info', (req, res) => {
  res.json({
    ...
    server: {
      ...
      version: '1.0.0',
      environment: NODE_ENV,
      platform: IS_VERCEL ? 'Vercel Serverless' : 'Traditional Server',
      websockets: !IS_VERCEL && io ? 'supported' : 'not supported',
      port: PORT
    },
    status: {
      mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected',
...
// Get all active locations (simplified for Vercel)
app.get('/api/admin/locations', (req, res) => {
  ...
  res.json({
    ...
    note: IS_VERCEL ? 'Location data is temporary and resets between function calls' : undefined
  });
});
...
app.get('/api/test', (req, res) => {
  res.json({ 
    ...
    timestamp: new Date().toISOString(),
    environment: NODE_ENV,
    platform: IS_VERCEL ? 'vercel-serverless' : 'local'
  });
});

// Location update endpoint (HTTP fallback for when WebSockets aren't available)
app.post('/api/location/update', (req, res) => {
  const { userId, userName, userType, lat, lng, speed } = req.body;
...
// Socket.IO connection handling (only if not on Vercel)
if (!IS_VERCEL && io) {
...
// Only call init if not on Vercel
if (!IS_VERCEL) {
  init();
}
